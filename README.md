...
- **Password reset** – a customer who forgets their password requests a link from the sign-in page and sets a new one on `/reset-password`. The link is single use, and the request answers the same way whether or not the address is registered, so the form cannot be used to discover which emails have accounts.
