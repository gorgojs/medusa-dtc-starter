...
- **Transactional emails** – an SMTP notification provider sends [React Email](https://react.email/) templates through `nodemailer`, driven by subscribers for `customer.created`, `auth.password_reset`, `order.placed`, `order.completed`, `order.fulfillment_created`, `order.transfer_requested`, and `payment.captured`. Every email reaches the buyer in the language they shopped in.
