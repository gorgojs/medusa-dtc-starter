...
export default async function orderCompletedEmailHandler({
  ...
  const html = await render(createElement(OrderCompletedEmail, { order }));
  const text = [
    `Заказ #${order.display_id ?? order.id} выполнен. Спасибо за покупку!`,
    "",
    `Следить за заказами: ${(process.env.STOREFRONT_URL || "https://rustarter.example").replace(/\/$/, "")}/${process.env.STOREFRONT_DEFAULT_COUNTRY || "ru"}/account/orders`,
  ].join("\n");
  ...
  try {
    await notificationService.createNotifications({
      ...
      content: {
        subject: `Заказ #${order.display_id ?? order.id} выполнен — gorgojs`,
        html,
        text,
      },
    } as any);
    ...
    logger.info(
      `[order-completed-email] Sent to ${order.email} for order ${order.id}`,
    );
  } catch (err: any) {
    logger.error(
